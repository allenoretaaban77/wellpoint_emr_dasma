...
$dstart  = $_GET["start"];
$dend  = $_GET["end"];
if($paid != 0){echo "<h1>Total Paid = 0</h1>
                    <a href='bcreport?task=generate&hmoid={$hmoid}&start={$dstart}&end={$dend}'>Back</a>";
                    }else{
...
$hmo_form_ids = implode(",", $hmo_form_ids);
    ...
    $query = "select * from hmo_form_items
     where hmo_form_id in($hmo_form_ids) AND itemid not in (SELECT form_itemid from hmoar_chkapply)";   
    $command=$connection->createCommand($query);
$datareader=$command->query();
if ($datareader){
...
<h1>HMO Billing & Collection Report - Not Paid Transactions</h1>
...
if ($_GET["Search"]){
    ...
    //    $criteria->addCondition("itemid in ". "(select form_itemid from hmoar_chkapply where check_id = (select checkid from hmoar_checks where check_no = '".$search_check_number."'))");   
    //}        
}
