...
//bill total
 $query =   "select sum(a.charge_fee) as bill_total
            ...
            where a.claim_doctor_id = $docid
            and a.payto = 'DOCTOR'                    
            and c.date_prepared between '$dstart' and '$dend' ";
             $command=$connection->createCommand($query);
            $datareader=$command->query();
            if ($datareader){
                foreach($datareader as $recd) {
...
<?php $this->renderPartial('doctrnxs_search',array(
    'model'=>$model,
)); ?>
</div>
   ...
   <!-- <script type="text/javascript">
    ...
    <a id='Download'
    target='_blank'
     href='http://<?= $_SERVER["HTTP_HOST"] ?>/hmoarreports/reports/hmodoctrnxexcel/?<?php
     echo "docid={$docid}&start={$dstart}&end={$dend}&paid={$paid}&";
