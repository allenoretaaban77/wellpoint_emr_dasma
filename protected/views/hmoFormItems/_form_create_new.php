...
<?php $form=$this->beginWidget('CActiveForm', array(
    ...
    <?php
    if ($_SESSION["errflag"] == true){                         
        echo "<span class='error'>".$_SESSION["errmsgs"]."</span>" ;
        echo "<div id='errdiv' style='display:none;'>";        
        foreach($_SESSION['error_list'] as $errlist){
            echo "<br/><span class='error'>- ".$errlist."</span>" ;
        }
        echo "</div><br/><button id='errbtn'>Show Errors</button>";
        $_SESSION["errmsgs"] = "";
        $_SESSION["errflag"] = false;
        $_SESSION['error_list'] = "";
    }                       
                       ?> 
    ...
    <div class="row">                        
        ...
        <?php echo $form->dropDownList($model,'payto',array('WPCLINIC'=>'WellPoint Clinic','DOCTOR'=>'Doctor'), array('empty'=>'Select Payable To')); ?>
    ...
    <div class="row">
        ...
        <?php echo $form->dropDownList($model,'claim_doctor_name', $a); ?>       
        <!--<?php 
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#select').click(function(){
                ...
                calculate();                    
                $('#ref_product_id').val("");   
                $('#ref_product_name').val("");
                $('#HmoFormItems_name').text("");
                $('#HmoFormItems_name').val("");
                $('#ref_product_price').val("");                        
                return false;
            });
        ...
        function med_service_to_array(){
            
            var array_service = new Array(); 
            var i = 0;               
            $('.service .service_name').each(function(index, value) {
            
               array_service[i] = encodeURIComponent($(this).text());
               i++;
            });
    ...
    <div class="row">
        ...
        <?php echo $form->dropDownList($model,'service_type',array('DIAGNOSTIC'=>'Diagnostic','CONSULTATION'=>'Consultation','APE'=>'Annual Physical Exam'), array('empty'=>'Select a Diagnosis')); ?>
...
<?php echo $form->dropDownList($model,'charge_type',array('CCHARGE'=>'Clinic Charge','PROCEDURE'=>'Procedure','PROF_FEE'=>'Professional Fee'), array('empty'=>'Select a Charge Type')); ?>
