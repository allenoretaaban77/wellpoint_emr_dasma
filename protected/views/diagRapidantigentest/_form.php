...
<?php 
    ...
    if(!$_POST["patientval"]){    
        $patientid = $_POST["DiagRapidantigentest"]['patient_id'];
    }else{       
...
?>
    ...
    <div class="row">
        <?php 
            echo $form->labelEx($model,'date_released');
            ...
            echo $form->error($model,'date_released'); 
        ?>
    </div>
    ...
    <div class="row">
        ...
        <input type="hidden" name="DiagRapidantigentest[created_by_userid]" value="<?=$created_by_userid ?>">
    </div>       
    ...
    <div class="row" style="display:none;">
        <?php echo $form->labelEx($model,'patient_id'); ?>
        <input type="hidden" name="DiagRapidantigentest[patient_id]" value="<?=$patientid ?>">
        <?php echo $form->textField($model,'patient_id',array('size'=>20,'maxlength'=>20,'value'=>$patientid)); ?>
        <?php echo $form->error($model,'patient_id'); ?>
    </div>
    
    <div class="row" style="display:none;">
        <input type="hidden" name="DiagRapidantigentest[date_created]" value="<?= date("Y-m-d h:m:s")?>">        
    </div>

    <div class="row buttons">
        <?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
        <?php echo CHtml::link(CHtml::button('Cancel'),array('diagRapidantigentest/admin'),array('style'=>'text-decoration:none;')); ?>
    </div>
