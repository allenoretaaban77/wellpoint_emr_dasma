...
function ajaxApplyToAll(row, totalRow, unappliedAmount, applied, soa, CheckNo, hacPercent){
    if(row <= totalRow){
        if(applied <= unappliedAmount){
            $.ajax ({
                url: "<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=searchUnapplied&soa="+soa
                    +"&cw_total="+unappliedAmount+"&CheckNo="+CheckNo,
                dataType: "json",
                success: function ( data, textStatus, jqXHR ) {
                    if(data.itemid == null){
                        ...
                        itemid=  data.itemid
                        charge_fee = parseInt(data.charge_fee);
                        $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?p="+hacPercent+"&a=wtax&fee="+charge_fee, function(result) {
                            wtax = parseInt(result);
                            itemid =  itemid;
                            app_amnt = charge_fee - wtax;
                            app_tax = wtax;
                            provider_xces = null;
                            member_xces = null;
                            hmo_xces =null;
                            hmo_xces_rem = null;
                            applied = parseInt(applied) + charge_fee;
                            if(applied <= unappliedAmount)
                            {

                                $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=checkid&check_no="+CheckNo, function(result) {
                                    check_id = result;
                                    params = itemid + "|"+app_amnt + "|"+ app_tax+ "|"+ check_id +
                                        "|"+ provider_xces +
                                        "|"+ member_xces +
                                    ...
                                    $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=SaveApply&params=" + params, function(result) {
                                        row = parseInt(row) + 1;
                                        $('#status').html("applied item: "+row+" of " + totalRow+"   <br/>check applied: "+ applied + " of "+unappliedAmount);
                                        ajaxApplyToAll(row, totalRow, unappliedAmount, applied, soa, CheckNo, hacPercent);
                                    });
                                });
                            }
                            else{
...
function applyToAll(hacPercent){

    var soa = $('#ataBtn').val();
    //var unappliedCheckAmount = parseInt($('#unappliedCheckAmount').val());
    //var unappliedWtaxAmount = parseInt($('#unappliedWtaxAmount').val());
    var unappliedCheckAmount = parseFloat($('#unappliedTotal').val());
    var CheckNo = $('#unappliedCheckNo').val();
    //var cw_total = unappliedCheckAmount + unappliedWtaxAmount;
    var cw_total = unappliedCheckAmount;

    $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=checkbilled&soa="+soa,function(result) {

        bill = result;
        if(parseInt(bill) <= cw_total){
            ...
            var r=confirm("The check amount is insufficient to apply all billed amount \n\n\n\n Are you sure?");
            if (r==true){
                x="OK";
            }else{
                x="CANCEL";
        ...
        $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=countcheckbilled&soa="+soa,function(result) {
            totalRow = result;
            $('#status').text(totalRow);
            row = 0;
            applied =0;
            if(x =="OK"){
                ajaxApplyToAll(row, totalRow, cw_total, applied, soa, CheckNo, hacPercent);
            }
        });
    });
...
/***
 * REVIEW the applySubmit() and the  validateApply function
 *
 *
 *
 *
 */
function QuickCheckApply(itemid, fee, pname, medsvc, percent){
    charge_fee = parseInt(fee.replace(',',''));
    //unappliedCheckAmount = parseFloat($('#unappliedCheckAmount').val())+parseFloat($('#unappliedWtaxAmount').val());
    unappliedCheckAmount = parseFloat($('#unappliedTotal').val());

    if(charge_fee <= unappliedCheckAmount){
        var r=confirm("Do you want to use Quick Apply for "+percent+"% Tax?");
        if (r==true){
            x="OK";
        }else{
            x="CANCEL";
        ...
        alert('Not Enough Check Amount!');
        x = "CANCEL";
    }
    if(x == "OK") {
        $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?p="+percent+"&a=wtax&fee="+charge_fee, function(result) {
            wtax = result;
            itemid =  itemid;
            app_amnt = charge_fee - wtax;
            app_tax = wtax;
    ...
    /*        result = false;
     $.get("<?= Yii::app()->request->baseUrl; ?>/sync/HmoarSync.php?a=wtax", function(result) {
     wtax_percent = result;

     wtax = fee*(wtax_percent/100);
